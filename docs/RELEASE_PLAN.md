...
| **0.4.x** | Patch releases for the review follow-ups | Each patch closes at least one of #9–#16 with a synthetic test; no new feature | — |
...
> Run workflow, which opens the v0.4.0 draft because `main` already
> carries version 0.4.0) or push the tag `v0.4.0` yourself.
...
* `CHANGELOG.md` has the version's section; overclaims found in review
  are corrected in the same release (for 0.4.0: the zh-CN catalog covers
  three of seven profiles, see #14).
...
| Issue | Area | Severity |
| --- | --- | --- |
| #9 | `compare`: frequency-response delta interpolates the raw spectrum before smoothing | medium |
| #10 | `analyze_impulse_response` / `analyze-ir` untested | medium |
| #11 | session loader follows absolute member paths | low (security) |
| #12 | loopback FIR window advances the compensated response by ~5 ms | medium |
| #13 | PortAudio: progress inside the real-time callback; swallowed callback errors | medium |
| #14 | zh-CN catalog incomplete; safety warning untranslated; `.mo` not shipped | high for the i18n claim |
| #15 | ISO 3382-2 Table 1: source unnamed; engineering class ignores microphone count | medium |
| #16 | `check_src_safety.py` gaps; coverage figure unrecorded | low |
